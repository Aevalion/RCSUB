...
#function to re-enable all movement related buttons
def EnableMovement():
    upbutton.config(state = 'normal')
    downbutton.config(state = 'normal')
    straightbutton.config(state = 'normal')

#function to disable all movement related buttons
def DisableMovement():
    upbutton.config(state = 'disabled')
    downbutton.config(state = 'disabled')
    straightbutton.config(state = 'disabled')

#issues a command to move servos up and disables movement commands while moving
def Up():
    try:
        ssh.exec_command("sudo python servoup.py")
        DisableMovement()    
        window.after(1000, EnableMovement)
    except:
        messagebox.showerror("Error", "Unable to communicate with the pi.\nPlease check your connection.")
        


#issues a command to move servos down and disables movement commands while moving
def Down():
    try:
        ssh.exec_command("sudo python servodown.py")
        DisableMovement()
        window.after(1000, EnableMovement)
    except:
        messagebox.showerror("Error", "Unable to communicate with the pi.\nPlease check your connection.")
...
#issues a command to re-adjust servos at normal level
def Straight():
    try:
        ssh.exec_command("sudo python servonormal.py")
        DisableMovement()
        window.after(500, EnableMovement)
    except:
        messagebox.showerror("Error", "Unable to communicate with the pi.\nPlease check your connection.")
...
def Start_Video():
    ...
    try:
        ...
        try:
            #opens vlc and starts running checks after 1s to see when it closes
